    ...
    return (
        <>
            <div className={`w-full h-full rounded-lg bg-blue relative text-white z-[60] shadow-lg shadow-gray-500  dark:shadow-darkMode-black border border-gray-400`} style={{ fontFamily: 'TidFont' }} >
                ...
                <div className='absolute top-5 right-16 text-[10px]' >{"حساب بانک تجارت"}</div>
                {/* <div className='text-xs absolute top-[6.5rem] right-5'>{t('ACCOUNT_NUMBER')}</div> */}
                <div className='text-md absolute top-[4.5rem] left-[50%] -translate-x-[50%]' style={{ fontFamily: 'CardNumber', direction: 'ltr', letterSpacing: 3 }} >{itm?.accountNumber}</div>
                <div className='text-xs font-semibold absolute top-32 right-5'>{t('ACCOUNT_BALANCE')}</div>
                <div className='text-xs flex absolute font-semibold top-32 left-5'>
                    <div>
                        {handleDataJsx(itm?.availableBalance?.amount, 'balance')}
                    </div>
                    <div className='mr-1 font-normal'>
                        {"ریال"}
                    </div>
                </div>
                <div className='text-[11px] absolute  bottom-2 left-5' style={{ direction: 'ltr' }}>{handleDataJsx(itm, "date")}</div>
            </div>
        </>
    );
}
